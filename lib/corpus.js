...
function addRoleToUserInfo({
  ...
  return db.get(userid)
    .then((body) => {
      const userold = body;
      console.log(`${new Date()} These are the users's roles before adding/removing roles.${JSON.stringify(userold.roles)}`);
    ...
    .then((userPermission) => {
      console.log(`${new Date()} User roles updated.`, userPermission);
      return { userPermissionResult: userPermission };
    })
    .catch((err) => {
      console.log(`${new Date()} failed to change ${userPermission.username} user's roles due to couchdb error `, userPermission, err);
      return { ok: false };
    });
}
/*
