  ...
  return server.db.create(corpusObject.dbname)
    .then((response) => {
      ...
      return addRoleToUserInfo({
        ...
        .catch((couchDBError) => {
          console.warn(couchDBError, `${new Date()} There was an couchDBErroror in creating the docs for the user's new corpus:`);
...
function addRoleToUserInfo({
  ...
  return db.get(userid)
    ...
    .catch((err) => {
      console.error({ err, userPermission }, `${new Date()} failed to change ${userPermission.username} user's roles due to couchdb error`);
      throw err;
    });
}
/*
...
function isRequestingUserAnAdminOnCorpus({
  ...
  return usersdb.get(`org.couchdb.user:${username}`)
    ...
    .catch((err) => {
      ...
      err.userFriendlyErrors = err.userFriendlyErrors || ['There was a problem deciding if you have permission to do this.'];
      throw err;
    });
}
