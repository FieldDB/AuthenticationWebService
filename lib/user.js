...
function createCouchDBUser({
  ...
  userParamsForNewUser.roles = userParamsForNewUser.roles.sort((a, b) => a - b);
  return usersdb.insert(userParamsForNewUser, userid)
    ...
    .catch((couchDBError) => {
      console.log(`${new Date()} User activity feed replication failed.`, couchDBError);
    })
...
function findByUsername({
  ...
  return usersdb.get(username)
    .then((result) => {
      ...
      console.error(err, `${new Date()} No User found: ${username}`);
      throw err;
    })
    .catch((error) => {
      if (error.error === 'not_found') {
        console.warn(error, `${new Date()} No User found: ${username}`);
      ...
      if (error.error === 'unauthorized') {
        console.error(error, `${new Date()} Wrong admin username and password`);
...
function findByEmail({
  ...
  return usersdb.view('users', usersQuery)
    ...
    .catch((error) => {
      console.error(`${new Date()} Error in findByEmail while quering ${usersQuery} ${JSON.stringify(error)}`);
...
function addRoleToUser({
  ...
  return corpus.isRequestingUserAnAdminOnCorpus({
      ...
      .map((userPermission) => {
        ...
        return corpus.addRoleToUserInfo({ connection: dbConn, userPermission })
          ...
          .catch((error) => {
            ...
            error.userFriendlyErrors = [`Unable to add ${userPermission.username} to this corpus.${error.status === 404 ? ' User not found.' : ''}`];
            throw error;
          });
      });
    ...
    .catch((err) => {
      console.log(err, 'error isRequestingUserAnAdminOnCorpus');
      throw err;
    });
  // });
}
  ...
  return findByUsername({ username, req })
    ...
    .then(({ user }) => {
      ...
      try {
        ...
        console.error(e, "Couldnt convert the users' sync details into a user.");
      }
...
function registerNewUser({
  ...
  return findByUsername({
    ...
    .catch((err) => {
      ...
      const connection = new Connection(req.body.connection || req.body.couchConnection) || Connection.defaultConnection(user.appbrand);
      ...
      return createCouchDBUser({
        ...
        .then(({ couchUser, whichUserGroup }) => {
          debug('user creaeted', couchUser, whichUserGroup);
        ...
        .then((couchresponse) => {
          ...
          user.corpora = [connection.toJSON()];
          emailFunctions.emailWelcomeToTheUser(user);
          /*
         * The user was built correctly, saves the new user into the users database
         */
