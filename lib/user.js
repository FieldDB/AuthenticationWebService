        ...
        .map((userPermission) => {
          ...
          return corpus.addRoleToUserInfo({ connection: dbConn, userPermission })
            .then(({ userPermissionResult }) => {
              console.log('corpus.addRoleToUserInfo result', userPermissionResult);
            ...
            .catch((error) => {
              console.log(' Bringing in mesages and status from failing user result.', error);
              error.userFriendlyErrors = [`Unable to add ${userPermission.username} to this corpus.`];
