  ...
  return Promise.all(corpora.map((potentialnewcorpusconnection) => {
    ...
    return corpus.createNewCorpus({
      ...
      .then(({ corpusDetails, info } = {}) => {
        console.log('Create new corpus results', corpusDetails.description, info);
        // if (err.status === 302) {
  ...
  return findByUsername({
    ...
    .catch((err) => {
      ...
      const userParamsForNewUser = {
        ...
        name: user.username,
        password,
        roles: [`${corpusDetails.connection.dbname}_${admin}`,
          `${corpusDetails.connection.dbname}_${contributor}`,
          `${corpusDetails.connection.dbname}_${collaborator}`,
          `${corpusDetails.connection.dbname}_${commenter}`,
          `${'public-firstcorpus' + '_'}${collaborator}`,
          'fielddbuser',
          `${corpusDetails.connection.brandLowerCase}user`,
        ],
        type: 'user',
      };
      /*
      ...
      return usersdb.insert(userParamsForNewUser, userid)
        .then((couchUser) => {
          debug('user creaeted', couchUser, userParamsForNewUser);
          return corpus.createNewCorpus({
