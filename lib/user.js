...
function addRoleToUser({
  ...
  return new Promise((resolve, reject) => {
    corpus.isRequestingUserAnAdminOnCorpus(req, req.body.username, dbConn, (error, result, messages) => {
      if (error) {
        req.log.error(error, 'error isRequestingUserAnAdminOnCorpus');
        return reject(error);
      }
      debug(`${new Date()} User ${req.body.username} is admin and can modify permissions on ${dbConn.dbname}`);
      ...
      const promises = req.body.users.map((userPermission) => {
        if (userPermission.add && typeof userPermission.add.map === 'function') {
          userPermission.add = userPermission.add.map((role) => {
            console.log('normalizeRolesToDBname', role);
            return normalizeRolesToDBname({ dbname: dbConn.dbname, role });
          });
        } else {
          userPermission.add = [];
        }
        if (userPermission.remove && typeof userPermission.remove.map === 'function') {
          userPermission.remove = userPermission.remove.map((role) => {
            console.log('normalizeRolesToDBname', role);
            return normalizeRolesToDBname({ dbname: dbConn.dbname, role });
          });
        } else {
        ...
        .map((userPermission) => {
          console.log('done preparing userPermission', userPermission);
          /*
           * If they are admin, add the role to the user, then add the corpus to user if succesfull
           */
          return new Promise((resolveAddRole, rejectAddRole) => {
            corpusmanagement.addRoleToUser(dbConn, userPermission, (error, resultOfAddedToCorpusPermissions, info) => {
              debug('corpusmanagement.addRoleToUser ', resultOfAddedToCorpusPermissions, info);

              return addCorpusToUser({ username: userPermission.username, userPermissionResult: resultOfAddedToCorpusPermissions, newConnection: dbConn })
                .then(({ userPermissionResult, info }) => {
                  debug(' Bringing in mesages and status from successful user result.', info);
                  resolveAddRole(userPermissionResult);
                });
