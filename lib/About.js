...
module.exports.FieldDB = function FieldDB(args) {
  ...
  this.getVersion = function getVersion(req, res, next) {
    res.send('getVersion');
  };
  /*
   ____  _           _     _          _
	...
	*                                      */
  this.installFieldDB = function installFieldDB(req, res, next) {
    ...
    next = function next(info, result, error) {
      if (error) {
        res.send(info);
      } else {
        res.send(result);
