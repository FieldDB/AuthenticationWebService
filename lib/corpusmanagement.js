...
module.exports.createDbaddUser = function createDbaddUser(dbConnection, user, successcallback, errorcallback) {
   ...
   */
  const usersdb = c.database('_users', () => {
  ...
  usersdb.save(userid, userParamsForNewUser, (err, doc) => {
    if (doc === undefined) {
      doc = {
        error: err,
      };
...
const addRoleToUser = function addRoleToUser(dbConnection, userPermission, successcallback, errorcallback) {
...
module.exports.changeUsersPassword = function changeUsersPassword(dbConnection, user, newpassword,
  ...
  usersdb.get(userid, (err, doc) => {
    ...
    // Delete the user
    usersdb.remove(userid, userold._rev, (err, couchdbresponse) => {
