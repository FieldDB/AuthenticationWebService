...
const addDeprecatedRoutes = function addDeprecatedRoutes(app) {
  ...
  app.post('/login', (req, res, next) => {
    userFunctions.authenticateUser({
      ...
      .catch((err) => {
        debug(err, `${new Date()} There was an error in the userFunctions.authenticateUser`);
        next(err);
      });
  });
  ...
  app.post('/forgotpassword', (req, res, next) => {
    ...
    userFunctions.forgotPassword({ email, req })
      .then(({ info }) => {
        const returndata = {};
        returndata.info = [info.message];
        debug(returndata, `${new Date()} Returning forgotpassword success`);
        res.status(200);
        res.send(returndata);
      })
      .catch((err) => {
  ...
  app.post('/corpusteam', (req, res, next) => {
    ...
    userFunctions.fetchCorpusPermissions({
      ...
      debug(returndata.users.readers, `${new Date()} Returning the list of reader users on this corpus as json:`);
      res.send(returndata);
    })
      .catch((err) => {
      // res.status(cleanErrorStatus(err.statusCode || err.status) || 400);
      // returndata.status = cleanErrorStatus(err.statusCode || err.status) || 400;
        debug(err, `${new Date()} There was an error in the userFunctions.fetchCorpusPermissions`);
        // returndata.userFriendlyErrors = [info.message];
        next(err);
      });
  });
   ...
   * as json
   */
  function addroletouser(req, res, next) {
    ...
    userFunctions.authenticateUser({
      ...
      .catch((err) => {
        ...
        // debug(`${new Date()} There was an error in the
